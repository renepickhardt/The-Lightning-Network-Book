...
\newglossaryentry{autopilot}{name={Autopilot},
  description={The autopilot is a recommendation engine for Lightning Network Nodes using statistical measures about the known topology to suggest with which nodes a particular node shoul open a channel.
    Depending on the implementation of the autopilot also the channel capacity is recommended.
...
\newglossaryentry{balance}{name={Channel Balance},
  description={The balance of a Payment Channel is encoded by the most recent \gls{commitment transaction}.
    The Balance states how many Bitcoin belong to each channel partner and how many Bitcoin are in flying\footnote{HTLCs which are currently in the routing process and have not been settled yet.} htlcs.
    The sum of the balance sheet equals the \gls{capacity}.
    The channel balance is only known by the channel partners.
    If the channel balance is completly on one side this particular channel partner can not receive any payments through this channel but can only send payements or forward htlcs.
    We call such a balance outbound capacity.
    The other channel partner would speek of inbound capacity.
    Obviously a node should aim the have balanced channel balances.}}
\newglossaryentry{mining}{name={Bitcoin Mining},
  description={Bitcoin mining serves two purposes.
    Firstly Bitcoin Transactions are validated and fixed in the sense that they cannot be doublespent.
    Secondly new Bitcoins are being created within the coinbase of ech block.\todo{Maybe extend}}}
\newglossaryentry{blockchain}{name={Blockchain},
  description={The Blockchain is an irreversible distributed database storing all Bitcoin transactions.
    The irreversibility comes from the fact that each block is only validated by solving a Proof of Work riddle and including the hash of the previous block.
    Thus the data can only be changed by an entity providing more than 51\% of the computational power of the Bitcoin network.
    Blocks are 1 Megabyte in size and storing Bitcoin transactions.
    New blocks are generated statistically every 10 Minutes.}}
...
\newglossaryentry{capacity}{name={Channel Capacity},
  description={The capacity of a payment channel is equivilent to the amount of bitcoin provided by the funding transaction.
    As the funding transaction is publicly visible on the blockchain and the channel is announced via the gossip protocol the capacity is a public information.
    It does not reveal any information about which of the channel partners own how many Bitcoin in the channel.
    A high capacity does not guarntee that the channel can be used for routing in both directions.\footnote{Acually the capacity does not even change if some of the capacity is locked up in htlcs. This means that even a high capcity channel could be unable to route a payment at a certain amount of time in both directions.}
...
\newglossaryentry{coinbase}{name={Coinbase},
  description={The first Transaction in each Bitcoin block is allowed to have an empty input script and transfer a certain amount of Bitcoin to some address.
    The amount is part of the consensus rules and depends on the current block height.
    Additionally to the newly generated coins the miner is also allowed to add all the fees of the transactions from the current block to the coinbase transaction.}}
...
\newglossaryentry{gossip protocol}{name={Gossip Protocol},
  description={Lightning Network nodes send and recieve information about the \gls{topology} of the Lightning Network through gossip messages which are exchanged with their peers.
    The gossip protocol is mainly defined in BOLT 7 and defines the format of the node\_announcement, channel\_announcment and channel\_updete messages.
    In order to prevent SPAM node anouncement messages will only be forwarded if the node already has a channel and channel anounce messages will only be forwarded if the funding transaction of the channel has been confirmed by the bitcoin network.
    Usually Lightning Nodes peer with their channel partners but it is fine to peer with any other Lightning node in order to process gossip messages.}}
...
\newglossaryentry{htlc}{name={Hashed Time Lock Contract},
    description={Additional output in the commitment transactions, which can be claimed by one node if a secret preimage to a hash is provided within the time lock after the commitment transaction is being mined by the Bitcoin network.
...
\newglossaryentry{preimage}{name={Preimage},
  description={In mathematics given a function $f$ and a value $h$ the preimage of $h$ with respect to $f$ is the set of values $R = \{r_1,r_2,...\}$ such that $f(r_i) = h$ for all $\r_i \in R$.
    In layment terms it is the set of values which are mapped to $h$ by the function $f$.
    This preimage set can be empty, finite or infinit.
    ...
    In pariticular when using SHA-256 we should state that each element has an infinite amount of preimages.
...
\newglossaryentry{rsmc}{name={Revocable Sequence Maturity Contract},
  description={This \gls{contract} is used to construct a payment channel between two Bitcoin users who do not need to trust each other.
    The name comes from a sequence of states which are encoded as commitment transactions and can be revoked if wrongfully published and mined by the Bitcoin netowork.
    These contracts are commonly referred to as RSMCs.}}
\newglossaryentry{topology}{name={Topology},
  description={The topology of the Lightning Network describes the shape of shape of the Lightning Network as a mathematical graph.
    Nodes of the graph are exactly the Lightning Network nodes or participants.
...
\newglossaryentry{transaction}{name={Transaction},
  description={Transactions are a binary format used by the Bitcoin protocol to transfer Bitcoins from one address to another one.
    Several transactions build a block which has to be confirmed by the Bitcoin Network through the process of \gls{mining}.
    Transactions can only be included to a block if they contain a valid signature defined by the input script or previous owner.
    The first Transaction in each block is called the \gls{coinbase} and generates new Bitcoins.
    Transactions can also build a \gls{contract} and shall not be confused with \gls{payments}.}}
\newglossaryentry{source based routing}{name={Source-Based Routing},
  description={
    On the Lightning Network a sender of the payment decides the route of the Payment.
    While this decreases the success rate of the routing process it increases the privacy of payments.
    Due to the SPHINX Mix Format used by the Onion Routing all routing nodes do not no the originator of a payment and the final recipient.
...
\newglossaryentry{node}{name={Lightning Network Node},
  description={
    ...
    Funds on a Lightning Network Node are in a hot wallet which is the only way one reasonably use the lightning network.
    ...
    On the Lightning Network this is not be possible without custodial services.
...
\newglossaryentry{ephemeral key}{name={Ephemeral Key},
  description={
    ...
    Even if a ephemeral key leakes only information about a single payment become public.
}}
\newglossaryentry{diffie hellman}{name={Diffie Hellman Key Exchange},
  description={
    ...
    An example of the derived key would be the ephemeral key during used by the SPHINX mix format.
...
\newglossaryentry{sphinx}{name={SPHINX Mix Format},
  description={A particular technique for Onion Routing used in the Lightning Network and invented by George Danezis and Ian Goldberg in 2009.
    ...
    While protecting the privacy of the sender and receiver of the network each node is able to return an error message along the path to the originator of the message.\footnote{https://cypherpunks.ca/~iang/pubs/Sphinx_Oakland09.pdf}
}}
...
\newglossaryentry{onion routing}{name={Onion Routing},
  description={
    ...
    With the SPHINX Mix Format also the final destination remains anonymous as only the previous router could see it but does not know if this is the final node or just a next hop.
...
\newglossaryentry{breach remedy}{name={Breach Remedy Transaction},
  description={A transaction claiming the outputs of a revocable sequence maturity contract with the help of the recokation key.
...
\newglossaryentry{closing transaction}{name={Closing Transaction},
  description={If both channel partners agree to close a channel they will create a spent of the Funding Transaction that reflects the most recent Commitment Transaction.
   ...
   After exchanging signatures for a Closing Transaction no further channel updates should be made as this one allow one side to enforce the closing transaction on the blockchain.
Mutualy closing a channel with the help of a closing transaction has the advantage that less blockchain transactions are required to claim all funds in comparison to unilaterally forcing a channel close by publishing a commitment transaction.
}}
\newglossaryentry{commitment tx}{name={Commitment Transaction},
  description={Commitment Transactions encode the balance of the Payment Channel with the help of one output for each channel partner by spending the funding transaction.
    When payments are being made or forwarded through the channel a doublespend of the commitment transactions is made by creating a new pair of commitment transactions.
    One Output also holds a Revocable Sequence maturity contract which is made to deincentivice a channel partner to broadcast an old commitment transaction to the Bitcoin Network.
    This effectively invalidates old doublespends.
...
\newglossaryentry{funding tx}{name={Funding Transaction},
  description={The Funding Transaction is used to open a Payment Channel.
    From the perspective of the bitcoin network the  process of opening a channel by creating a \gls{rsmc} is started by creating the funding transaction and finnished by broadcasting it to the bitcoin network and have it included to the blockchain.
    The value of the Funding Transaction is exactly the capacity of the payment channel.
    The output of the Funding Transaction is a 2 out of 2 multisig script where each channel partner controlls one key.
...
\newglossaryentry{revocation key}{name={Revocation Key},
  description={Each Revokable Sequence Maturity Contract contains two Revocation Keys.
    ...
    Knowing both Revokation Keys the Output of the Revokable Sequence Maturity Contract can be spent within the predifined Timelock.
    Revokation Keys are used to disincentivice channel partners from broadcasting an old channel state.
...
\newglossaryentry{payment channel}{name={Payment Channel},
  description={Payment Channels are the core building block of the Lightning Network.
    They can be used to send a predefined amount of bitcoin back and forth between two parties.
    Sending Bitcoins over a payment channel happens off chain so only the Funding Transaction of the Payment channel is stored in the blockchain and the settlement with the final balance that spends the Funding Transaction is stored in the blockchain.
    ...
    Joseph Poon and Tadge Dryja describe the Recocable Sequence Maturity Contract based method that is currently being implemented on the Lightning Network in the Lightning Network Whitepaper.
...
\newglossaryentry{lnd}{name={lnd},
  description={Implementation of the Lightning Network Protocol by the San Francisco based company lightninglabs.
It is written in Go.}}
